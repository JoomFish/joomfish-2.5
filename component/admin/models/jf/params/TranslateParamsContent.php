...
class TranslateParamsContent extends TranslateParams
{
	...
	function editTranslation()
	{
		...
		/*
		if we want change some params here i the place
		this must do if we have an new item and 
		*/
		/*
		...
		foreach($this->fields as $field)
		{
			switch($field->Name)
			{
				...
				case 'metadata':
					...
					{
						$this->forms->orig_form->bind(array($field->Name => json_decode($field->originalValue)));
					}
				...
				case 'hits':
					...
					if ($field->originalValue != "")
					{
						$this->forms->orig_form->bind(array($field->Name => $field->originalValue));
					}
					*/
					$this->values['translated'][$field->Name] = $field->translationContent->value;
					//if($field->translationContent->value != "")
					//{
						//this fields are empty on new
		...
		if(version_compare(JVERSION, '2.5', 'ge'))
		{
			...
			<?php if ($params['show_urls_images_backend']): ?>
				<?php echo JHtml::_('sliders.panel',JText::_('COM_CONTENT_FIELDSET_URLS_AND_IMAGES'), 'urls_and_images-options'); ?>
					...
					/* ?>
						<li>
							<?php $this->forms->outputFieldParams($this->forms->trans_form->getField('images')); ?>
						</li>
				...
				<fieldset class="panelform joomfish_panelform">
					<ul class="adminformlist">
						...
						<li>
							...
							$this->forms->orig_form->setFieldAttribute('created_by', 'filter', 'unset'); ?>
							<?php $this->forms->orig_form->setFieldAttribute('created_by', 'readonly', 'true'); ?>
							<?php $this->forms->orig_form->setFieldAttribute('created_by', 'class','readonly'); 
							*/?>
						...
						<li>
							...
							/*
							we cant set this here, why?
							$this->forms->orig_form->setFieldAttribute('created_by_alias', 'type', 'text'); ?>
							<?php $this->forms->orig_form->setFieldAttribute('created_by_alias', 'readonly', 'true'); ?>
							<?php $this->forms->orig_form->setFieldAttribute('created_by_alias', 'class','readonly'); */?>
						...
						<?php if ($this->forms->trans_item->translated->modified_by || $this->forms->orig_item->translated->modified_by) :///if ($this->values['translated']['modified_by']) : ?>
						<li>
					...
					<?php if ($this->forms->trans_item->translated->version || $this->forms->orig_item->translated->version) ://if ($this->values['translated']['version']) : ?>
						<li>
							<?php $this->forms->outputFieldParams($this->forms->trans_form->getField('version'),false); //no buttons here ?>
						</li>
					<?php endif; ?>

					<?php if ($this->forms->trans_item->translated->hits || $this->forms->orig_item->translated->hits) ://if ($this->values['translated']['hits']) : ?>
						<li>
							<?php $this->forms->outputFieldParams($this->forms->trans_form->getField('hits'),false); //no buttons here ?>
						</li>
					<?php endif; ?>
